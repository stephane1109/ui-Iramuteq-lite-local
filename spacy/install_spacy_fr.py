#!/usr/bin/env python3
"""Installe spaCy + un modèle français (par défaut fr_core_news_sm).
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--model", default="fr_core_news_sm", help="Nom du modèle spaCy à installer.")
    p.add_argument("--repo-url", default=None, help="Alias de --index-url (compatibilité).")
...
def main() -> int:
    ...
    py = sys.executable
    repo_args: list[str] = []
    effective_index_url = args.index_url or args.repo_url
    if effective_index_url:
        repo_args.extend(["--index-url", effective_index_url])
