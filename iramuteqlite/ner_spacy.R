...
installer_spacy_si_necessaire <- function(model = "fr_core_news_sm") {
  ...
  if (!requireNamespace("spacyr", quietly = TRUE)) {
    try(utils::install.packages("spacyr"), silent = TRUE)
  }
  if (!requireNamespace("spacyr", quietly = TRUE)) {
    ...
    if (requireNamespace("remotes", quietly = TRUE)) {
      try(remotes::install_github("quanteda/spacyr"), silent = TRUE)
    }
  }
  if (!requireNamespace("spacyr", quietly = TRUE)) {
    options(iramuteq_spacy_install_last_log = "spacyr indisponible après tentative d'installation.")
    return(FALSE)
  }

  ok <- tryCatch({
    spacyr::spacy_install(lang_models = model, prompt = FALSE)
    spacyr::spacy_initialize(model = model)
    TRUE
  }, error = function(e) {
    logs <<- c(logs, paste0("spacyr install/init error: ", e$message))
    FALSE
  })
  if (length(logs)) options(iramuteq_spacy_install_last_log = paste(logs, collapse = "\n"))
  isTRUE(ok) && spacy_modele_disponible(model = model)
}
